...
from .ncm_ctf_code import CausalGraph
from .data import ExpertDataset

'''Graph utility.'''
def parse_graph(nodes: [], edges: []) -> CausalGraph:
    V = [n['name'] for n in nodes]
    directed_edges = [
        (e['from_'], e['to_']) for e in edges if e['type_'] == 'directed'
    ]
    bidirected_edges = [
        (e['from_'], e['to_']) for e in edges if e['type_'] == 'bidirected'
    ]
