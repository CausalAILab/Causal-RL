...
from typing import TypeVar, Optional, Iterable, Tuple
from typing import Set, FrozenSet, Dict, Sequence, AbstractSet
from networkx.drawing.nx_agraph import graphviz_layout

from causal_gym.core import Graph

def graph_to_vizdag(graph: Graph):
    nodes = graph.nodes
    edges = graph.edges
    return CausalGraph(
        V = list(map(lambda x: x['name'], nodes)),
        DE = list(map(lambda x: (x['from_'], x['to_']), filter(lambda x: x['type_']=='directed', edges))),
        BDE = [(x['from_'], x['to_'], f'U{i}') for i, x in enumerate(filter(lambda x: x['type_']=='bidirected', edges))],
    )
